...
const Sidebar = () => {
  ...
  const {designs, isVerified, setIsViewHistory, setSelectedDesign, setIsImageLoaded} = useContext(AppContent)
    ...
    <div className={`h-screen bg-[#2E5E4E] transition-all ease-in-out duration-300 overflow-y-scroll scrollbar-hide ${collapsed ? 'w-16' : 'w-64'}`}>
      ...
      <div className={`text-[#D1E7D4] ${collapsed ? 'hidden' : 'block'}`}>
        ...
        <div className="flex justify-center font-bold mt-3">
          <h1 className={`${isVerified ? 'text-[#F5E3C7] text-2xl mt-5' : 'hidden'}`}>Your Designs</h1>
        </div>
        <div className="mt-3">
          {designs.length === 0 ? null : (
            filteredDesigns.length > 0 ? (
              ...
              <div className="text-center text-[#D1E7D4] mt-4">No matching results</div>
            )
          )}
        </div>
      </div>
