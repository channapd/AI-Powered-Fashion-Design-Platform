...
const Navbar = () => {
  ...
  const {userData, backendUrl, setUserData, setIsLoggedin, credit, setDesigns, setIsVerified, setSelectedDesign, setCredit, setIsViewHistory, setIsImageLoaded} = useContext(AppContent)
      ...
      {userData ? (
        <div className='flex items-center gap-2 sm:gap-3'>
            ...
            <div className='absolute hidden group-hover:block top-0 right-0 z-10 text-black rounded pt-10'>
              <ul className='list-none m-0 p-2 bg-[#AACBAF] text-sm'>
                {!userData.isAccountVerified && (
                  <li
                    onClick={!isSendingMail ? sendVerificationOtp : undefined}
                    className={`py-1 px-2 cursor-pointer ${
                      isSendingMail ? 'text-gray-500 cursor-not-allowed' : 'hover:bg-[#96BFA1]'
                    }`}
                  >
                    {isSendingMail ? 'Sending OTP...' : 'Verify email'}
                  </li>
                )}

                <li onClick={()=>navigate('/settings')} className='py-1 px-2 hover:bg-[#96BFA1] cursor-pointer pr-10'>Settings</li>
          ...
          <div className='flex items-center gap-2 sm:gap-5'>
            ...
            <button onClick={handleLoginClick} className='bg-[#2E5E4E] hover:bg-[#1E4638] text-[#D1E7D4] font-bold py-2 px-4 rounded-full'>
              Login
            </button>
