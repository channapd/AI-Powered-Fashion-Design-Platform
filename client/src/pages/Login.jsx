...
import { faEnvelope, faL, faLock, faSpinner, faUserCircle } from '@fortawesome/free-solid-svg-icons';
    ...
    <div className='flex items-center justify-center min-h-screen px-6 sm:px-0'>
      <div className='bg-[#2E5E4E] p-10 rounded-lg shadow-lg w-full sm:w-96 text-[#2E5E4E] text-sm'>
        ...
        <form onSubmit={onSubmitHandler}>
          ...
          <button
            type="submit"
            className={`w-full py-2.5 rounded-full font-medium flex items-center justify-center transition duration-300 ${
              isSpinning
                ? 'bg-[#A9C8B1] cursor-not-allowed'
                : 'bg-[#D1E7D4] text-[#2E5E4E]'
            }`}
            disabled={isSpinning}
          >
            {isSpinning ? (
              <FontAwesomeIcon icon={faSpinner} spin className='text-[#2E5E4E]' />
            ) : (
              state
            )}
          </button>
